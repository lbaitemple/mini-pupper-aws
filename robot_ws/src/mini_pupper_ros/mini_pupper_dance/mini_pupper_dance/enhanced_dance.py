...
import time
import asyncio
from .util import enhanced_parse_movement_string
...
class MiniPupperMusicClientAsync(Node):
    ...
    def send_request(self, cmd, filename, start):
        self.req.command = cmd
        self.req.file_name = filename
        self.req.start_second = start
        self.get_logger().info('music service send {}'.format(self.req))
...
class DanceDemo(Node):
    ...
    def dance(self):
        ...
        for readcommand in self.commands:
            ...
            result = enhanced_parse_movement_string(readcommand)
